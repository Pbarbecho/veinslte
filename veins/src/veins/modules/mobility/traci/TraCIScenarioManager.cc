...
void TraCIScenarioManager::init_traci() {
	{
		...
		if ((apiVersion == 10) || (apiVersion == 11) || (apiVersion == 1)) {
			MYDEBUG << " Pablo modified TraCI server \"" << serverVersion << "\" reports API version " << apiVersion << endl;
		}
		else {
...
void TraCIScenarioManager::addModule(std::string nodeId, std::string type, std::string name, std::string displayString, const Coord& position, std::string road_id, double speed, double angle) {
	...
	//Pablo capture IPs
	int mac_id = macNodeId;
	CaptureIPAddress(nodeId,  mod->getFullName(),  idToAddress[nodeId], mac_id);
...
void TraCIScenarioManager::CaptureIPAddress(std::string NodeID, std::string FullName, IPv4Address idtoaddress, int macID)
{
    IP_File.open(filename, std::ios::out | std::ios::app);                                    //para leer datos ios::in
    if (IP_File.is_open()){
        //SREQ_MSG << "Node" << "|" << "HOPS" << "|" << "Time(sec)"<<'\n';
        IP_File << NodeID << "," << FullName << "," << idtoaddress << "," << macID<< "," << simTime().dbl()<<'\n';
        IP_File.close();
    }
